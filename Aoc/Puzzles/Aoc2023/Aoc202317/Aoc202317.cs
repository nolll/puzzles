...
public class Aoc202317(string input) : AocPuzzle
{
    ...
    private static List<Graph.Input> BuildGraph(Matrix<int> matrix, int minStraightCount, int maxStraightCount)
    {
    ...
    private static List<Graph.Input> GetDirectionGraph(Matrix<int> matrix, MatrixAddress coord, MatrixDirection forward, int minStraightCount, int maxStraightCount)
    {
        matrix.MoveTo(coord);
        ...
        for (var i = 1; i <= maxStraightCount; i++)
        {
            ...
            if (i >= minStraightCount && !matrix.IsOutOfRange(leftCheckCoord))
                graph.Add(new Graph.Input(fromId, $"{leftCoord.Id}|{left}|1", matrix.ReadValueAt(leftCoord)));
            if (i >= minStraightCount && !matrix.IsOutOfRange(rightCheckCoord))
                graph.Add(new Graph.Input(fromId, $"{rightCoord.Id}|{right}|1", matrix.ReadValueAt(rightCoord)));
            if (i < maxStraightCount && !matrix.IsOutOfRange(forwardCoord))
                graph.Add(new Graph.Input(fromId, $"{forwardCoord.Id}|{forward}|{i + 1}", matrix.ReadValueAt(forwardCoord)));
        }
