...
namespace ConsoleApp.Years.Year2015.Puzzles
{
    public class Day13 : Day2015
    {
        ...
        {
            WritePartTitle();
            var table = new DinnerTable(LegacyInput);
            Console.WriteLine($"Happiness change: {table.HappinessChange}");

            WritePartTitle();
            var table2 = new DinnerTable(LegacyInput, true);
            Console.WriteLine($"Happiness change including me: {table2.HappinessChange}");
        }

        protected override string LegacyInput => @"Alice would lose 57 happiness units by sitting next to Bob.
